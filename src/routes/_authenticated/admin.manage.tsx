...
function SectionsManager() {
  ...
  useEffect(() => { load(); setSelected(new Set()); setSelectMode(false); /* eslint-disable-next-line */ }, [bookId]);
  ...
  const bulkDelete = async () => {
    const ids = Array.from(selected);
    if (ids.length === 0) return;
    if (!confirm(`Delete ${ids.length} section${ids.length > 1 ? "s" : ""}?`)) return;
    const { error } = await supabase.from("book_sections").delete().in("id", ids);
    if (error) { toast.error(error.message); return; }
    toast.success(`Deleted ${ids.length}`);
    setSections((p) => p.filter((s) => !selected.has(s.id)));
    exitSelect();
  };

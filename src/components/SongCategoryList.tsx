...
import { BackButton, EmptyState } from "./ui-bits";
...
import type { Book, Song, SongCategory } from "../lib/types";

const LABELS: Record<SongCategory, { en: string; hi: string }> = {
  church: { en: "Church Song Book", hi: "कलीसिया गीतमाला" },
  additional: { en: "Additional Songs", hi: "अतिरिक्त गीत" },
};

export function SongCategoryList({ category }: { category: SongCategory }) {
  ...
  const inCategory = useMemo(() => {
    ...
    return list.filter((s) => (s.category ?? "church") === category);
  }, [songsQ.data, category]);

  const filtered = useMemo(() => {
    if (!q.trim()) return inCategory;
  ...
  const shellTitle = language === "hi" ? label.hi : label.en;

  return (
    <AppShell
      title={shellTitle}
      left={<BackButton to="/books/song-book" />}
    >
      <div className="pt-3">
